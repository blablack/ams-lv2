...
#include "vcdoubledecay_gui.hpp"

VCDoubleDecayGUI::VCDoubleDecayGUI(const std::string& URI)
{
	...
	p_offsetFrame->pack_start(*(m_dialRatioOffset	= CreateDial("Ratio",	p_ratioOffset,		NORMAL, 0.01)));
	...
	p_gainFrame->pack_start(*(m_dialRatioGain		= CreateDial("Ratio",	p_ratioGain,		NORMAL, 0.01)));
...
LabeledDial* VCDoubleDecayGUI::CreateDial(const std::string TextLabel, p_port_enum PortIndex, DialType Type, double Step)
{
	...
	p_tempDial->signal_value_changed().connect(compose(bind<0>(mem_fun(*this, &VCDoubleDecayGUI::write_control), PortIndex), mem_fun(*p_tempDial, &LabeledDial::get_value)));

	return p_tempDial;
}


void VCDoubleDecayGUI::port_event(uint32_t port, uint32_t buffer_size, uint32_t format, const void* buffer)
{
	switch(port)
	{
		case p_attackOffset:
		...
		case p_sustainOffset:
			...
			break;
		case p_ratioOffset:
			m_dialRatioOffset->set_value(*static_cast<const float*> (buffer));
			break;
		...
		case p_sustainGain:
			...
			break;
		case p_ratioGain:
			m_dialRatioGain->set_value(*static_cast<const float*> (buffer));
			break;
		case p_releaseGain:
...
static int _ = VCDoubleDecayGUI::register_class("http://github.com/blablack/ams-lv2/vcdoubledecay/gui");
