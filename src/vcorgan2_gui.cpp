...
#include "vcorgan2.hpp"

#if OSC_COUNT == 4
#include "vcorgan2_4_ttl.hpp"
#elif OSC_COUNT == 6
#include "vcorgan2_6_ttl.hpp"
#elif OSC_COUNT == 8
#include "vcorgan2_8_ttl.hpp"
#endif
...
#include "vcorgan2_gui.hpp"

VCOrgan2GUI::VCOrgan2GUI(const std::string& URI)
{
	...
	for( int i = 0; i < MODULE_DYNAMICWAVES_OSC; i++ )
	{
		...
		m_comboWaveForm[i]->signal_changed().connect(mem_fun(*this, &VCOrgan2GUI::get_waveform));
		...
		p_freqFrame->pack_start(*(m_scaleSemitone[i]	= CreateDial("Semitone", semitone[i], NORMAL, 1)));
...
void VCOrgan2GUI::get_waveform()
{
	for( int i = 0; i < MODULE_DYNAMICWAVES_OSC; i++ )
			this->write_control(waveForm[i], m_comboWaveForm[i]->get_active_row_number());
	}

LabeledDial* VCOrgan2GUI::CreateDial(const std::string TextLabel, p_port_enum PortIndex, DialType Type, double Step)
{
	...
	p_tempDial->signal_value_changed().connect(compose(bind<0>(mem_fun(*this, &VCOrgan2GUI::write_control), PortIndex), mem_fun(*p_tempDial, &LabeledDial::get_value)));

	return p_tempDial;
}

void VCOrgan2GUI::port_event(uint32_t port, uint32_t buffer_size, uint32_t format, const void* buffer)
{
	switch( port )
	{
		case p_octave:
	...
	for( int i = 0 ; i < MODULE_DYNAMICWAVES_OSC ; i++ )
	{
		...
		else if( port == semitone[i] )
			m_scaleSemitone[i]->set_value(*static_cast<const float*> (buffer));
		else if( port == phi0[i] )
...
#if OSC_COUNT == 4
	static int _ = VCOrgan2GUI::register_class("http://github.com/blablack/ams-lv2/vcorgan2_4/gui");
#elif OSC_COUNT == 6
	static int _ = VCOrgan2GUI::register_class("http://github.com/blablack/ams-lv2/vcorgan2_6/gui");
#elif OSC_COUNT == 8
	static int _ = VCOrgan2GUI::register_class("http://github.com/blablack/ams-lv2/vcorgan2_8/gui");
#endif
