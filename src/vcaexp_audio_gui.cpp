...
VCAExpAudioGUI::VCAExpAudioGUI(const std::string& URI)
{
    ...
    m_dialGain1->signal_value_changed().connect(compose(bind<0>(mem_fun(*this, &VCAExpAudioGUI::write_control), p_gain1), mem_fun(*m_dialGain1,  &LabeledDial::get_value)));
    p_gainBox->pack_start(*m_dialGain1);

    m_dialGain2 = new LabeledDial("2nd Gain Boost", p_gain2, 0, 1, NORMAL, 0.01, 2);
    m_dialGain2->signal_value_changed().connect(compose(bind<0>(mem_fun(*this, &VCAExpAudioGUI::write_control), p_gain2), mem_fun(*m_dialGain2,  &LabeledDial::get_value)));
    ...
    m_dialIn1->signal_value_changed().connect(compose(bind<0>(mem_fun(*this, &VCAExpAudioGUI::write_control), p_in1), mem_fun(*m_dialIn1,  &LabeledDial::get_value)));
    p_volumeBox->pack_start(*m_dialIn1);

    m_dialIn2 = new LabeledDial("In 2", p_in2, 0, 2, NORMAL, 0.01, 2);
    m_dialIn2->signal_value_changed().connect(compose(bind<0>(mem_fun(*this, &VCAExpAudioGUI::write_control), p_in2), mem_fun(*m_dialIn2,  &LabeledDial::get_value)));
    ...
    m_dialOutputLevel->signal_value_changed().connect(compose(bind<0>(mem_fun(*this, &VCAExpAudioGUI::write_control), p_outputLevel), mem_fun(*m_dialOutputLevel,  &LabeledDial::get_value)));
...
static int _ = VCAExpAudioGUI::register_class("http://github.com/blablack/ams-lv2/vcaexp_audio/gui");
